...
'''
print(f"Using gym version: {gym.__version__}", flush=True)
...
for _ in range(1): # Reduced to 1 for faster script startup
    done = False
    action = 1
    obs = env.reset()
    total_reward = 0
    while not done:
        ...
        total_reward += reward
        # env.render() # Commented out for faster testing
        obs = obs_
...
'''
    Question 2: Implement DDQN and Dueling-DDQN for frogger-v1.
''' 
...
if __name__ == '__main__':
    ...
    MODE = "plot_and_video"  # Change to "train" to run training
    # ------------------------------------------------------------------

    # Define file paths
    PLOT_DATA_FILE = "./figures/q2_plot_data.npz"
    BEST_MODEL_INFO_FILE = "./models/q2_best_model_info.json" # For video

    # Define Hyperparameters (same as Q1 for a fair comparison)
    hyperparameters = {
        "BUFFER_SIZE": int(1e5),
        "BATCH_SIZE": 64,
        "GAMMA": 0.99,
    ...
    model_configs = [
        {"name": "DQN", "ddqn": False, "dueling": False},
        {"name": "DDQN", "ddqn": True, "dueling": False},
        {"name": "Dueling_DDQN", "ddqn": True, "dueling": True}
    ]
    ...
    if MODE == "train":
        print("===== STARTING TRAINING MODE =====", flush=True)
        
        # Store raw results before processing
        raw_model_results = {}
        global_min_len = np.inf

        # --- NEW: Track best score PER MODEL and OVERALL ---
        best_score_per_model = {config["name"]: -np.inf for config in model_configs}
        best_seed_per_model = {config["name"]: None for config in model_configs}
        
        global_best_score = -np.inf
        
        for config in model_configs:
            model_name = config["name"]
            print(f"\n--- Training Model: {model_name} ---", flush=True)
            all_eval_scores_raw = []
            ...
            for seed in seeds:
                ...
                _, eval_scores, current_eval_steps, best_score_for_seed = train_dqn(
                    env, agent, seed, model_name=model_name, n_episodes=10000, eval_every=10000
                )

                # --- NEW: Track bests ---
                # Check if this is the best run for this MODEL TYPE (e.g., best DQN)
                if best_score_for_seed > best_score_per_model[model_name]:
                    best_score_per_model[model_name] = best_score_for_seed
                    best_seed_per_model[model_name] = seed
                ...
                if best_score_for_seed > global_best_score:
                    ...
                    best_model_info_for_video = {"config": config, "seed": seed, "score": best_score_for_seed}
            ...
            # Store raw scores
            raw_model_results[model_name] = all_eval_scores_raw

        print("\n===== TRAINING COMPLETE. PROCESSING RESULTS... =====", flush=True)
        
        # --- BUGFIX: Process all results AFTER training, using global_min_len ---
        eval_steps_axis = eval_steps_axis[:int(global_min_len)]
        save_plot_data = {'eval_steps_axis': eval_steps_axis}

        for model_name, raw_scores_list in raw_model_results.items():
            ...
            if eval_scores_np.size == 0:
                print(f"Warning: No scores found for model {model_name}. Skipping.", flush=True)
                continue
            ...
            # Store for plotting
            processed_model_results[model_name] = (mean_scores, std_scores)
            
            # Store for saving to file (mean, std)
            save_plot_data[model_name] = (mean_scores, std_scores)
        ...
        # --- NEW: Print info for model submission ---
        print("\n===== Best Models for Submission =====", flush=True)
        for model_name, best_seed in best_seed_per_model.items():
            if best_seed is not None:
                score = best_score_per_model[model_name]
                print(f"  > Best '{model_name}': Seed {best_seed} (Score: {score:.2f})", flush=True)
                print(f"    - Submit file: ./models/{model_name}_checkpoint_{best_seed}.pth", flush=True)
            else:
                print(f"  > No successful runs for '{model_name}'.", flush=True)

    elif MODE == "plot_and_video":
        print(f"===== LOADING DATA FROM FILES FOR PLOTTING & VIDEO =====", flush=True)
    ...
    if processed_model_results:
        ...
        for model_name, (mean_scores, std_scores) in processed_model_results.items():
            # Check dimensions one last time
            if len(eval_steps_axis) != len(mean_scores):
                print(f"WARNING: Mismatch for {model_name}. X: {len(eval_steps_axis)}, Y: {len(mean_scores)}", flush=True)
                # Truncate mean/std to match x-axis (should not happen with the fix, but good safety)
                min_len = min(len(eval_steps_axis), len(mean_scores))
                ax.plot(eval_steps_axis[:min_len], mean_scores[:min_len], label=model_name)
                ax.fill_between(eval_steps_axis[:min_len], std_scores[:min_len], std_scores[:min_len], alpha=0.2)
            else:
                ax.plot(eval_steps_axis, mean_scores, label=model_name)
                ax.fill_between(eval_steps_axis, mean_scores - std_scores, mean_scores + std_scores, alpha=0.2)
        
        ax.set_ylabel('Average Score', fontsize=14)
        ax.set_xlabel('Training Steps', fontsize=14)
        ax.set_title('DQN vs. DDQN vs. Dueling-DDQN Performance (frogger-v1)', fontsize=16)
        ax.legend(fontsize=12)
        ax.grid(True)
        
        plot_filename = './figures/q2_all_models_evaluation.png'
    ...
    # --- Final Evaluation and Video Recording (Uses OVERALL best model) ---
    if best_model_info_for_video:
        ...
        if not best_model_config or best_seed is None:
            print("Error: Best model info file is incomplete. Skipping video.", flush=True)
        else:
            print(f"\n--- Recording final performance of OVERALL best model: {best_model_config['name']} (Seed: {best_seed}) ---", flush=True)
            ...
            model_path = f"./models/{best_model_config['name']}_checkpoint_{best_seed}.pth"
            ...
            try:
                ...
                video_path = f'./videos/final_overall_best_{best_model_config["name"]}'
